  ...
  function deleteRating(ratingId) {
    fetch(`${backendUrl}/api/v1/ratings/${ratingId}`, { method: "DELETE" })
      ...
      .then((deletedRating) => {
        setRezept({
          ...rezept,
          ratings: rezept.ratings.filter((rating) => rating._id !== deletedRating._id),
        });
      })
      .catch((err) => {
        console.log(err);
  ...
  if (!rezept) return <p>Loading...</p>;
    ...
    return (
      <>
        <main>
          <h1>{rezept.name}</h1>
          <img src={`${backendUrl}/${rezept.bildUrl}`} alt="Rezept Cover" width={800} />
          <p>{rezept.anleitung}</p>

          <div>
            <button onClick={() => setPortionenCount(portionenCount - 1)}>-</button>
            <span style={{ fontWeight: "bold", margin: 4 }}>{portionenCount}</span>
            <button onClick={() => setPortionenCount(portionenCount + 1)}>+</button>
          </div>
          <ul style={{ width: 200 }}>
            {rezept.zutaten.map((zutat) => (
              ...
              >
                <b>
                  {isNaN(zutat.menge) ? "" : ((zutat.menge / rezept.portionen) * portionenCount).toFixed(0)}
                  {zutat.einheit}
                </b>
                {zutat.zutat}
              </li>
